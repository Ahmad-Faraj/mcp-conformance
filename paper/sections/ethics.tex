...
CPU, and process counts, and no host filesystem access. The census was executed in the single-phase condition, in which install and
...
therefore receives the same isolation as the probe. Server code also had network access while being measured. To limit the risk, every container is discarded after a
...
for evidence that server code produced effects outside its container. At least \(1.3\%\) of servers attempted an
outbound connection that failed and logged an error; a successful connection leaves
no trace in the captured output, so this is a lower bound. Five servers emitted live
credential material into their own logs (session tokens they had generated), and
one auto-provisioned an API key and account against a third-party service, which is the class of effect our frame's exclusion of credential-requiring servers
was meant to prevent. We have redacted every such value from the released dataset and
asked the service to revoke the provisioned key and delete the account. We draw two
conclusions. First, excluding servers that \emph{declare} required
...
cost explicitly here.

\paragraph{Responsible disclosure}
Some of our findings concern robustness. A server that stops
serving after a malformed frame, and a server that emits non-protocol bytes on its
stdio channel, can each desynchronize or strand a connected client. We report these
...
repository metadata. We reported \(15\) of the \(17\) servers that stop serving after a
malformed frame to their maintainers through their issue trackers. At the time of
writing, five have merged a fix and four have shipped it in a release, and one
maintainer corrected our severity assessment, which we adopted
(Section~\ref{sec:threats}). The remaining findings concern unknown-tool handling
and stdout contamination and are of lower severity; we report them only in
aggregate. The identities of all \(211\) servers with security-relevant findings are
withheld from the released dataset.

\paragraph{Registry and maintainer impact}
Our crawl pages sequentially through the registry's public API, backing off on
errors, and our installs draw each package once per probe from its public registry
in the ordinary way. We view the released harness as a benefit to maintainers, since
it lets any author check their own server's conformance before publishing.
