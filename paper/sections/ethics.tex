...
repository metadata, and were reported to maintainers before publication. Of the
servers whose maintainers we could reach, two reproduced the behaviour independently
within a day: one shipped a fix in a released version, and one confirmed a fix for its
next release with a regression test. A third response corrected our severity
assessment, which we adopted (Section~\ref{sec:threats}). We take these as evidence
that the findings are actionable and that the reporting format was usable; where a
...
avoid redundant load. We view the released harness as a benefit to maintainers, since
it lets any author check their own server's conformance before publishing, which is
