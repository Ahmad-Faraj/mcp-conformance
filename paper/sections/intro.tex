...
the specification illustrates. We trace this near-universal divergence to its cause:
it is the default behavior of the official SDKs, inherited by the servers built on
them. That result arrives in the middle of a live debate: an open proposal would
...
an argument their own declared schema rejects and return a well-formed result: a
code-scanning tool asked to scan an integer replied \texttt{CLEAN}. No error code, no
\texttt{isError} flag, nothing a client could act on---and unlike the error-reporting
question, this one violates an explicit requirement that servers validate all tool
inputs. A further \MalformedDiesRate{} die on a malformed frame, a denial-of-service
primitive.
