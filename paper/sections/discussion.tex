...
with something older still. So this census records an installed base at the point its
mechanism left the specification. The distance between the two is the finding. A
server published today inherits whichever default its SDK shipped with, and the
revision that removes the handshake reaches those servers only when their authors
upgrade, which the version spread here suggests is slow.
