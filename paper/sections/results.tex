...
hangs, or exits without speaking the protocol. The failure is in the software, not
the packaging. A concurrent random draw of 400 npm
...
malformed frame. We report this as a robustness and conformance failure, not a
...
responding populations of 2,347 and 1,020 respectively. A behavior absent from one
...
error code and no \texttt{isError} flag. The schema that would have caught the call is
the one the server published and did not enforce.
...
(Figure~\ref{fig:sdk}, Table~\ref{tab:sdk}). The behavior tracks the SDK: the
...
treat it as such. It does show that the same default reaches past the official SDKs, into
third-party wrappers that inherit it.
A few SDK defaults, used directly or through wrappers, account for this behavior across the ecosystem.
...
\begin{figure}[t]
  ...
  error-as-result rate for unknown tools, by package registry, with 95\% Wilson
  intervals.}
