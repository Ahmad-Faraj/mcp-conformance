...
third-party systems and keeps the population reproducible. We report the exclusion of
...
\begin{itemize}
  ...
  \item \textbf{tools-call-unknown}: calling a non-existent tool must be rejected.
    ...
    example; it uses no RFC-2119 \textsf{MUST}/\textsf{SHOULD} keyword. A
    diverging response is therefore never scored as a ``violation''. We
...
reasons given in Section~\ref{sec:method-env}. Section~\ref{sec:threats} carries
...
Whether a server runs at all is RQ1, so we count every stage:
...
Runs the harness itself broke are a separate outcome, reported with the funnel and
