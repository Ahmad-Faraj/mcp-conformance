...
\emph{entire} frame---all \Nprobed{} eligible servers---so our measurement is a
complete census of the eligible population, not a sample.
...
Linux capabilities dropped, and no host filesystem mounts other than a package-cache
volume. The harness supports two probe conditions. In the \textbf{two-phase}
...
reasons given in Section~\ref{sec:method-env}; we state the consequences for validity
...
servers so that disk usage stays bounded across the full ecosystem. In a pilot on an
independent subset (roughly 200 servers per arm) we compared this condition against
the two-phase network-isolated condition and found no detectable difference on any
metric---handshake yield ($54\%$ vs.\ $57\%$), error-as-result prevalence ($87\%$
vs.\ $87\%$), and silent type non-enforcement ($7\%$ vs.\ $7\%$). We note that a
pilot of this size can only exclude differences larger than roughly $14$ percentage
points in handshake yield, so this is evidence of consistency rather than a
demonstration of equivalence; the conformance metrics, being far more tightly
clustered, are correspondingly better constrained. A small number
of probes ($\le 0.4\%$) were interrupted by an automated disk-guard that reclaims
space if the host approaches capacity; these are recorded conservatively as
non-handshaking and, being a property of the harness environment rather than the
server, do not bias conformance rates (which are computed over responding servers).
