...
do~\cite{mcpcrawler}. A companion dataset effort catalogues 3,238 candidate MCP
repositories on GitHub and classifies them by operational role, again from repository
evidence rather than behaviour~\cite{mcpgithubdataset}, and a parallel line studies
the \emph{application} side, characterizing how 1,723 MCP clients configure and
supervise the servers they consume~\cite{mcpapps}. Fault taxonomies take a third
route and mine GitHub issues~\cite{runtimefaults,realfaults}. They find that tool
faults and schema-enforcement problems dominate what users actually hit, which is the
same pair our harness measures directly rather than through bug reports. None of this
...
conformance. Sandboxed behavioral scanners and taint-style analyzers hunt
vulnerabilities in curated or deliberately vulnerable server
sets~\cite{sandboxscan,viper}, a security-invariants benchmark avoids live
...
Execution at scale is no longer rare. MCPZoo assembles 64,611 unique servers, of
...
under manual validation~\cite{mcpzoo}. Two differences separate that corpus from ours
and they matter for what each can measure. First, MCPZoo is built by \emph{repairing}
...
when a client installs it as published. Second, its population is mined from GitHub
repositories, whereas ours is the official registry, the surface a client actually
resolves against. That their corpus required a repair framework at all is independent
corroboration of our runnability finding. Conformance remains a separate question,
namely whether a server that is neither malicious nor vulnerable does what the
specification says.

The closest work is concurrent. Afsar draws a seeded random sample of 400 npm stdio
...
declared structure. We census both npm and PyPI and test behaviour, calling tools
...
implementation practice. Rather than argue which mechanism is correct, we measure what
...
execute local stdio servers and test behaviour against inputs. It addresses the
output-validation clause, and we address tool resolution and input enforcement. Both
studies support the same underlying point, that this proposal has been debated without
deployment data.
...
headline numbers, such as the claim that most listed servers are unmaintained,
without published methodology, data, or protocol-level testing. Ours is,
